...
import type { TelegramUser, TelegramAuthResponse, JWTPayload } from '@studsovet/server/shared'
import { parseJwtPayload, isJwtValid, getJwtExpiry, getUserFromJwt } from '~/utils/jwt'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const authenticate = async (): Promise<boolean> => {
    ...
    try {
      const response: TelegramAuthResponse = await api.authenticateTelegram(initData.value)
      ...
      authError.value = error.response?.data?.message || error.message || 'Ошибка авторизации'
      return false
    } finally {
