...
const { formFields, isLoading, error, fetchFormFields, submitFormResponse } = useForms();
...
onMounted(async () => {
  try {
    ...
    await fetchFormFields(props.formId);
    
    // Устанавливаем заголовок формы
    formTitle.value = `Форма #${props.formId}`;
    ...
    errorMessage.value = error.value || "Ошибка загрузки формы.";
  }
});
...
async function handleSubmit() {
  ...
  try {
    const res = await submitFormResponse(props.formId, answers);
    ...
    errorMessage.value = err.message || "Ошибка отправки. Попробуйте позже.";
  } finally {
