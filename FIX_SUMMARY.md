# Исправление проблемы с frontend контейнером - Резюме

## 🎯 Проблема

При запуске frontend контейнера на сервере, он запускался и сразу останавливался:

```
frontend-1  | /docker-entrypoint.sh: Configuration complete; ready for start up
Gracefully stopping... (press Ctrl+C again to force)
```

## 🔍 Причина

1. **Использовался nginx** вместо встроенного Nuxt.js сервера
2. **Неправильная конфигурация портов** - nginx слушал на порту 80, а docker-compose ожидал порт 3000
3. **Отсутствовали переменные окружения** для подключения к backend API

## ✅ Решение

### 1. Изменен Dockerfile

**Было:** nginx + статические файлы  
**Стало:** Node.js + встроенный Nuxt.js (Nitro) сервер

### 2. Обновлен nuxt.config.ts

Добавлен `routeRules` для проксирования API в production режиме

### 3. Добавлены переменные окружения

В docker-compose.yml на сервере нужно добавить переменные для frontend и backend

## 📁 Измененные файлы

### В репозитории (уже изменено):

- ✅ `applications/web-app/Dockerfile`
- ✅ `applications/web-app/nuxt.config.ts`

### На сервере (нужно изменить):

- ⚠️ `~/projects/studsovet/docker-compose.yml`

## 📖 Документация

Созданы следующие файлы с инструкциями:

1. **QUICK_DEPLOY_GUIDE.md** ⭐ - **НАЧНИТЕ С ЭТОГО!**

   - Краткое пошаговое руководство
   - Что нужно сделать на сервере
   - Ожидаемые результаты

2. **DOCKER_COMPOSE_EXAMPLE.md**

   - Пример конфигурации docker-compose.yml
   - Что именно нужно добавить/изменить

3. **SERVER_DEPLOYMENT_CONFIG.md**

   - Подробная документация
   - Разные варианты конфигурации
   - Troubleshooting

4. **ENVIRONMENT_SETUP.md**
   - Настройка переменных окружения
   - Как работает проксирование
   - Настройка CORS

## 🚀 Быстрый старт

### На локальной машине:

```bash
# 1. Закоммитить изменения
git add .
git commit -m "fix: use Nuxt built-in server instead of nginx"
git push origin master

# 2. Дождаться сборки в CI/CD
```

### На сервере:

```bash
# 1. Отредактировать docker-compose.yml
nano ~/projects/studsovet/docker-compose.yml
# Добавить переменные окружения (см. DOCKER_COMPOSE_EXAMPLE.md)

# 2. Обновить образы и перезапустить
cd ~/projects/studsovet
docker compose down frontend backend
docker pull registry.gitlab.kosygin-rsu.ru/studsovet/studform-trpc-vue/frontend:master
docker pull registry.gitlab.kosygin-rsu.ru/studsovet/studform-trpc-vue/backend:master
docker compose up -d frontend backend

# 3. Проверить логи
docker compose logs -f frontend backend
```

## 🌐 Ваши домены

- **Frontend:** https://webapp.studsovet.kosygin-rsu.ru
- **Backend API:** https://api.webapp.studsovet.kosygin-rsu.ru

## ✅ Проверка

После деплоя:

1. Откройте https://webapp.studsovet.kosygin-rsu.ru в браузере
2. Проверьте что приложение загружается
3. Откройте DevTools → Network
4. Проверьте что API запросы работают без ошибок CORS

## 📞 Если нужна помощь

1. Проверьте логи: `docker compose logs -f frontend backend`
2. Смотрите **SERVER_DEPLOYMENT_CONFIG.md** раздел "Частые проблемы и решения"
3. Убедитесь что переменные окружения установлены правильно:
   ```bash
   docker exec -it studsovet-frontend-1 env | grep -E "BACKEND|HOST|PORT"
   docker exec -it studsovet-backend-1 env | grep FRONTEND
   ```

## 📚 Дополнительная информация

- **ENV_URLS_GUIDE.md** - общее руководство по настройке URL между сервисами
- **applications/web-app/DOCKERFILE_README.md** - документация по Dockerfile

---

**Важно:** Все изменения в коде уже сделаны и готовы к использованию. Осталось только:

1. Закоммитить и запушить
2. Обновить docker-compose.yml на сервере
3. Перезапустить контейнеры

Удачи! 🎉
